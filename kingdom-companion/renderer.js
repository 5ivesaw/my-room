...
const STATUS_MODES = new Set(['auto', 'online', 'busy', 'sleeping', 'offline']);
...
const storedStatusMode = localStorage.getItem('kingdom.statusMode') || localStorage.getItem('kingdom.status') || 'auto';
els.status.value = STATUS_MODES.has(storedStatusMode) ? storedStatusMode : 'auto';
els.message.value = localStorage.getItem('kingdom.message') || '';
els.contactUid.value = localStorage.getItem('kingdom.contactUid') || '';
await window.kingdomDesktop.setStatusMode(els.status.value === 'auto' ? 'online' : els.status.value);
updatePresenceDot();
...
function updatePresenceDot() {
  const mode = STATUS_MODES.has(els.status.value) ? els.status.value : 'auto';
  els.presenceDot.className = currentUser ? (mode === 'auto' ? 'online' : mode) : '';
}
...
function selectedPresence(statusOverride = '') {
  const mode = STATUS_MODES.has(els.status.value) ? els.status.value : 'auto';
  const status = ['online','busy','sleeping','offline'].includes(statusOverride)
    ? statusOverride
    : (mode === 'auto' ? 'online' : mode);
  return {
    mode,
    status,
...
async function publishPresence({ statusOverride = '', announce = false, forceHeartbeat = false } = {}) {
  ...
  const { mode, status, message, contactUid } = selectedPresence(statusOverride);
  ...
  try {
    ...
    if (!statusOverride) {
      localStorage.setItem('kingdom.statusMode', mode);
      localStorage.setItem('kingdom.status', mode);
    ...
    updatePresenceDot();
    if (announce) say(mode === 'auto' ? 'Automatic presence active: online while the companion runs.' : `Throne status synchronized: ${status}.`);
    return true;
  } catch (error) {
...
window.kingdomDesktop.onPresencePulse(() => {
  if (currentUser) publishPresence({ announce: false, forceHeartbeat: true }).catch(() => {});
});
...
document.addEventListener('visibilitychange', () => {
  if (!document.hidden && currentUser) publishPresence({ announce: false, forceHeartbeat: true }).catch(() => {});
});
...
window.kingdomDesktop.onPrepareOffline(async () => {
  try {
    ...
    // The main-process heartbeat and the website stale timer remain the fallback.
  } finally {
